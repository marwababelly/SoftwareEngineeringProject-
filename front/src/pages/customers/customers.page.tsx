...
import carQueries from "../../apis/car/car.queries";
import PageTable from "../../components/common/page-table/page-table.component";
...
const renderHead: TRenderHeadAction = (item: string, index: number) => (
  <th key={index}>{item}</th>
);

const renderBody: TRenderBodyAction<ICustomer> = (
  item: ICustomer,
  index: number,
  first?: ReactNode,
  end?: ReactNode
...
const CustomersPage = () => {
  ...
  const { data } = carQueries.useCarQuery({
    pageSize: 50,
    pageNum: page,
  });
  ...
  const handleCustomerDelete = async (id: number) => {
    try {
      await carApi.removeCar(id);
    } catch (error) {
      console.log(error);
    }
  };

  const CustomerPageTable = PageTable<ICustomer>({
    title: "customers",
